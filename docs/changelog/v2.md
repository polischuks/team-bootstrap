...
Archived releases. The current major series lives in [CHANGELOG.md](../../CHANGELOG.md).

The 2.x line ran from the first pipeline/role split through the harness-context and roles-alive
work. It ends at 2.35.0; 3.0.0 is the milestone that made the review roles dispatchable and the
per-role floor enforcing (`specs/020-live-roles-and-harness-wiring`).
...
- **Three test-quality `verify-batch` gates — the floor, not the ceiling.** v2.15/2.16 made "a test was
  ...
  - **F1 red-touches-tests** — `bin/tdd-red.sh` refuses a red whose committed change
...
- **P9's red step is now harness-enforced, not a self-declared boolean.** P9 mandates "tests written
  ...
  - **`bin/tdd-red.sh`** (run at the red step) runs the project's AGENTS.md `Test:`
